...
class ConversationGenerator:
    """
    Generate red-teaming conversations between LLMs hosted on Together and OpenAI
    ...
    Attributes:
        ...
        victim_model_provider (str): Provider for the victim model ('together' or 'openai').
        together_client (Together): API client for Together models.
        openai_client (OpenAI): API client for OpenAI models.
    ...
    def __init__(self, topic: str, history_path: str = "conversation_history.json",
                 ...
                 victim_model_provider: str = "openai",
        ...
        self.victim_model_provider = victim_model_provider.lower()
        ...
        self.openai_client = OpenAI(api_key=os.environ.get("OPENAI_RT_KEY"))
    ...
    def call_planner(self, conversation_history: str, recent_chatbot_response: str) -> str:
        ...
        """
        planner_prompt_func = getattr(oai_prompt, f"get_planner_system_prompt_{self.planner_prompt_version}")
        response = self.together_client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": oai_prompt.get_planner_user_prompt_current(self.topic, conversation_history, recent_chatbot_response)}
            ]
        )
    ...
    def call_attacker(self, conversation_history: str, user_suggestion: str, recent_chatbot_response: str) -> str:
        ...
        """
        attacker_prompt_func = getattr(oai_prompt, f"get_attacker_system_prompt_{self.attacker_prompt_version}")
        response = self.together_client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": oai_prompt.get_attacker_user_prompt_current(conversation_history, user_suggestion, recent_chatbot_response)}
            ]
        )
        return response.choices[0].message.content
    ...
    def call_victim_model(self, conversation_history: List[MessageDict]) -> str:
        ...
        """
        if self.victim_model_provider == "openai":
